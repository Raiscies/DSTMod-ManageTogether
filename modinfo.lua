...
local trans = ChooseTranslationTable({
	{
		...
		description = [[通过GUI管理你的服务器.
在按住Tab(默认)打开的在线玩家信息窗口中, 管理员或者监督员的窗口左上角会出现一个打开历史玩家记录窗口的按钮,
点击打开GUI, 可以通过各种按钮管理服务器和在线或离线的玩家;
* 部分设置如果没有你想要的选项, 请到存档的模组配置文件中修改.
* 如果模组没有记录到一个玩家的信息, 那么模组将不能对该玩家进行操作. 例如在一个已存在的存档中途启用本模组, 那么在启用本模组之前曾加入过服务器的玩家将不会被记录到. 在启用模组之后玩家再加入服务器, 该玩家就会被成功记录.
* 监督员投票选项中的'一致同意'指没有投票反对的玩家, 但是允许弃权.
* 谨慎开启实验性功能, 因为它没有经过充分的测试而可能导致服务器崩溃. 并且有很大的限制: 只有在玩家在线时服务器才能检测模组过期,并且重启服务器实际上只是简单地关闭你的服务器, 只有你的服务器脚本在服务器关闭后能够自动重启才有用.
]],
		options = {
			yes = '是', 
			no = '否',
